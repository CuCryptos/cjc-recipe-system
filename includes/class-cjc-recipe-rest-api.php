...
class CJC_Recipe_REST_API {
    ...
    public static function get_recipe( $request ) {
        ...
        if ( ! $data ) {
            return new WP_Error(
                'recipe_not_found',
                __( 'Recipe not found.', 'cjc-recipe-system' ),
                array( 'status' => 404 )
            );
        }
    ...
    public static function get_recipe_by_parent( $request ) {
        ...
        return new WP_Error(
            'recipe_not_found',
            __( 'No recipe found for this post.', 'cjc-recipe-system' ),
            array( 'status' => 404 )
        );
    }
    ...
    public static function iso8601_to_time( $iso ) {
        ...
        $parts = array();
        if ( $hours > 0 ) {
            $parts[] = $hours . ' ' . _n( 'hour', 'hours', $hours, 'cjc-recipe-system' );
        }
        if ( $minutes > 0 ) {
            $parts[] = $minutes . ' ' . _n( 'minute', 'minutes', $minutes, 'cjc-recipe-system' );
        }
